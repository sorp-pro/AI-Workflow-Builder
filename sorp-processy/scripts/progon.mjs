...
// сбоев 0, неопределённостей 0, у каждого варианта решения есть путь и порог полноты пройден:
// всё из пакета процесса покрыто моделью или пропущено с причиной (polnota.mjs). Нет пакета —
// порог не считается и об этом пишется.
...
import { полнота, пакетПроцесса } from './polnota.mjs';
...
export function прогнатьМодель(модель) {
  ...
  const пакет = пакетПроцесса(итог.proc);
  итог.полнота = пакет ? полнота(модель, пакет) : null;
  итог.готово = !итог.дыры.length && !итог.сбои.length && !итог.неопределено.length && !итог.ветки.length && (модель.paths || []).length > 0 && (!итог.полнота || !итог.полнота.не_покрыто.length);
  return итог;
}
...
export function главная(argv = process.argv.slice(2)) {
  ...
  for (const ф of файлы) {
    ...
    if (один) {
      ...
      if (!итог.полнота) console.log('Полнота: пакета процесса нет — порог не считался (node paket.mjs ' + итог.proc + ')');
      else { console.log(`Полнота: покрыто ${итог.полнота.покрыто} из ${итог.полнота.всего}, пропущено с причиной ${итог.полнота.пропущено}, не покрыто ${итог.полнота.не_покрыто.length}`); for (const x of итог.полнота.не_покрыто) console.log(`  ✗ ${x.ref} — ${x.что}`); }
  ...
  if (!один) {
    ...
    for (const с of сводка) console.log(`${String(с.proc).padEnd(9)} ${с.ошибка ? 'ОШИБКА ' + с.ошибка : `ошибок ${с.проверка.ошибок} · сценариев ${с.сценариев} · дыр ${с.дыры.length} · неопр ${с.неопределено.length} · сбоев ${с.сбои.length} · ветки ${с.ветки.length} · непокрыто ${с.непокрытые.length} · текстом ${с.текстом.length} · вопросов ${с.вопросов} · полнота ${с.полнота ? `${с.полнота.покрыто}/${с.полнота.всего}, не покрыто ${с.полнота.не_покрыто.length}` : 'нет пакета'} · ${с.готово ? 'готово' : 'не готово'}`}`);
  }
}
